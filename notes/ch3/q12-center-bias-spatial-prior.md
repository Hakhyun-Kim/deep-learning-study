...
- **후속 — "중앙을 더 믿으면?"**: `w×중앙 + (1-w)×모서리평균`의 w 스윕 결과
  최적은 균등(w=0.2)이 아니라 **w=0.3 (77.98%)** — 중앙 편향이 실재하니
  가중치도 중앙 쪽으로 주는 게 맞다는, 원 질문의 직관을 확인. 단 w=0.3은
  test set을 보고 고른 값이라 +0.21%p는 과대평가일 수 있음(검증 split 필요).
  모서리만의 평균(w=0.0, 76.90%)이 중앙 단독(75.80%)을 이기는 앙상블 효과도 관찰.
