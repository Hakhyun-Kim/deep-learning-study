...
- exp03_aug_stronger: exp02 + ColorJitter/RandomErasing, test acc 75.05%
  (완료, ch3 — 과적합↓, 10에폭으론 덜 수렴)
- exp04_aug_epochs20: exp03 + 에폭 20, best 75.80% (ep12). +TTA = 77.77%.
  위치 민감도 실측: 중앙이 모서리보다 +7.1%p (photographer's bias)
- exp05_person_crop: 공식 bbox 1.5배 margin, 단독 70.79% (문맥 손실로 하락)
- 🎉 **최종 80.71% — 목표 80% 달성**: 사람시야(exp05) 0.3 + 전체시야(exp04)
  5-crop TTA 0.7 앙상블 (`scripts/two_view_ensemble.py --tta`)
- ch4(마무리) 문서 작성 완료 (notes/ch4/) — 학습자 회고·졸업 체크만 남음

## 연구(논문) 방향 논의 기록 위치

- SOTA 조사, 논문 전략, 연구 주제 같은 논의는 이 repo의 notes/가 아니라
  **로컬 private repo `D:\deep-learning-research`의 `topics/`에 기록**한다
  (그쪽 CLAUDE.md 규칙을 따를 것). 이 저장소는 공개 학습용으로 유지.
